...
import { TypeNote, TypeUid, TypeImage, TypeTag, TypeText, TypeList } from '../types'
...
type TypeActions = {
	...
	filterTags: (tag: TypeTag) => void,
	addCreateTag: (tag: string) => void,
	updateTag: (tag: TypeTag) => void,
	deleteTag: (key: TypeUid) => void,
	...
	addTagToNote: (noteKey: TypeUid, tagKey: TypeUid, isChecked: boolean) => void,
	addTagOfSelect: (tagKey: TypeUid, isChecked: boolean) => void,
...
const filterTags = (tag: TypeTag) => {
	update((state: TypeState) => {
		state.seletedTag = tag.key;
...
const addCreateTag = (tag: string) => {
	update((state: TypeState) => {
		if (!state.tags.includes(tag)) {
			state.tags.push({ key: uuid.v4(), tag });
		}

		postNote({ notes: state.notes, tags: state.tags });
		return state;
	})
}

const updateTag = (tag: TypeTag) => {
	update((state: TypeState) => {
		const indexTag = state.tags.findIndex((t) => t.key === tag.key);

		if (indexTag !== -1)
			state.tags[indexTag] = tag;
...
const deleteTag = (key: TypeTag) => {
	update((state: TypeState) => {
		const indexTag = state.tags.findIndex((t) => t.key === key);

		if (indexTag !== -1)
			state.tags.splice(indexTag);
...
const addTagToNote = (noteKey: TypeUid, tagKey: TypeUid, isChecked: boolean) => {
	update((state: TypeState) => {

		const indexNote = state.notes.findIndex(obj => obj.key === noteKey);

		if (indexNote !== -1) {
			const indexTag = state.notes[indexNote].tags
				.findIndex((_tag_: TypeTag) => _tag_.key === tagKey);

			if (isChecked && indexTag === -1) {
				state.notes[indexNote].tags.push(state.tags.find(t => t.key === tagKey));
			}

			if (!isChecked) {
				state.notes[indexNote].tags.splice(indexTag, 1);
			}
		}
...
const addTagOfSelect = (tagKey: TypeUid, isChecked: boolean) => {
	update((state: TypeState) => {
		state.selectedNotes.forEach((uid: TypeUid) => {
			const indexNote = state.notes.findIndex(obj => obj.key === uid);
